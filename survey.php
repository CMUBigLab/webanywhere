<?
if($_REQUEST[submit] === "Submit Responses") {
  $fp = fopen("survey_results.txt", "a+");
  fwrite($fp, "\n\n------------------------".date("g:i:s A")."-----------------------\n");    
...
<?
if($_REQUEST[submit] === "Submit Responses") { ?>
<title>Web Anywhere - Thanks!</title>
<body>
...
<? if($_REQUEST[age] && $_REQUEST[submit]) { ?>
...
<li>
<label for="q4a">With practice, I could access the web independently with this program.</label><br>
<select id="q4a" name="q4a">
<option value="no response">(no response)</option>
<option value="5 Strongly Agree">5 Strongly Agree</option>
<option value="4">4</option>
<option value="3">3</option>
