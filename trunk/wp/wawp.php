...
function show_overlimit() {
  include 'overlimit.php';
  exit(0);
}

function add_cookie($name, $value, $expires = 0) {
  // TODO:  Did it matter that I removed a . immediately following "domain="?
...
// Javascript needs to be rewritten in order to both make content work with the
// proxy and to make it.
function proxify_script($script) {
...
// Rewrite CSS to fetch content through the proxy.
...
// Define the encode/decode functions based on the flags that are set.
if($_flags['rotate13']) {
...
// TODO:  Make this do https connection when appropriate by looking at the URL of PHProxy.
...
do {
  // Silence the "normal" warning by a faulty SSL connection.
  $data = @fread($_socket, 8192);
  $_response_body .= $data;
}   
while (isset($data{0}));
...
if($_content_type == 'text/css') {
  ...
  $url_retrieved = $_url_parts['scheme'] . '://' . $_url_parts['host'];
  ...
  //
  // Remove some specific tags that can cause WebAnywhere to fail.
  //
  // Force gmail into HTML mode.
  $_response_body = str_replace('url = StripParam(url, "ui=html");', '', $_response_body);

  // Prevent gmail form setting start at the username login field.
  $_response_body = str_replace('onload="OnLoad()"', '', $_response_body);

  // Target should never be anything but the content frame.
  $_response_body = str_replace('<BASE TARGET="_top">', '', $_response_body);
      ...
      switch ($tag) {
        ...
        case 'base':
          if(isset($attrs['href'])) {
                    ...
                    // This removes the annoying <BASE TARGET="_top"> tag used by some sites.
                    if(isset($attrs['target'])) {
                      $attrs['target'] = preg_replace('#_top#is', 'content_frame', $attrs['target']);
                    }
                    break;
                case 'meta':
