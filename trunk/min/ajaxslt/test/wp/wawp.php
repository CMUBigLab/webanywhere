...
/*
if(isset($_GET['en']) && $_GET['en'] == 'n') {
  $_flags['base64_encode'] = 0;
 } else {
  $_flags['base64_encode'] = 1;
 }
*/
...
if ($_content_type == 'text/css') {
  ...
  // Record that content was retrieved.
  // Content that can be modified, HTML pages, count more.
  if($_limit_rate != 0 && isset($_SERVER['REMOTE_ADDR'])) {
    $ipArr = explode('.',$_SERVER['REMOTE_ADDR']);
    $ip = $ipArr[0] * 0x1000000
      + $ipArr[1] * 0x10000
      + $ipArr[2] * 0x100
  ...
  // Replace style files with proxified versions.
  preg_match_all('#(<\s*script[^>]*?>)(.*?)(<\s*/\s*script\s*>)#si', $_response_body, $matches, PREG_SET_ORDER);
  for($i = 0, $count_i = count($matches); $i < $count_i; ++$i) {
    $_response_body = str_replace($matches[$i][0], $matches[$i][1]. proxify_script($matches[$i][2]) . $matches[$i][3], $_response_body);
  }
  ...
  //$_response_body = preg_replace('#onload="[^"]*sf[^"]*"#i', '', $_response_body);
  ...
  if (!$_flags['show_images']) {
    ...
    // Process each opening tag that is found.
